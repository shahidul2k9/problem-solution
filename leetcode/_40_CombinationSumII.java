...
public class _40_CombinationSumII {
    ...
    private void bk(int pos, int runningSum, List<Integer> runningCombination, int[] candidates, int target, List<List<Integer>> combinationList) {
        ...
        if (val + runningSum <= target) {
            ...
            bk(pos + 1, runningSum + val, runningCombination, candidates, target, combinationList);
            runningCombination.remove(runningCombination.size() - 1);
            int skip = 1;
            while (pos + skip < candidates.length && candidates[pos] == candidates[pos+skip] ) skip++;
            bk(pos + skip, runningSum, runningCombination, candidates, target, combinationList);
        }
    }
